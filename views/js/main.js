...
function updatePositions( ) {
  for (var i = 0; i < items.length; i++) {
    items[i].style.transform = "translateX(" + ( 100 * Math.sin( scroll_Top + (i % 5) ) )  + "px)";
  }

  boolUpdate = true;
}

// updateScroll uses requestAnimationFrame on updatePositions. Set scroll_Top and boolUpdate are global.
...
function updateScroll ( ) {
  if(boolUpdate) {
      boolUpdate = false;
      scroll_Top = document.body.scrollTop / 1250;
      requestAnimationFrame( updatePositions );
  }
}

// runs updateScroll on scroll
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //the height of the each row is s = 256, so we compute the max num of rows and the num of pizzas needed
